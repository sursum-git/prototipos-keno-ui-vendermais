(function(){
    const LS_LAST_KEY  = "consultaFiltro:last";
    ...
    function collectFilters(){
        ...
        // Score robusto
        const scoreWidget = $("#fScore").data("kendoRangeSlider");
        let score = [0, 100];
        if (scoreWidget && typeof scoreWidget.values === "function") {
            score = scoreWidget.values();
        } else {
            const v = $("#fScore").val();
            if (v !== undefined && v !== null && v !== "") score = [Number(v), Number(v)];
        }
    ...
    function applyFilters(payload){
        ...
        if (Array.isArray(payload.score) && payload.score.length === 2){
            const w = $("#fScore").data("kendoRangeSlider");
            if (w && typeof w.values === "function") w.values(payload.score);
        }
    }
    ...
    function openRecoverWindow(){
      ...
      <div style="padding:12px;">
        <div style="font-size:12px; opacity:.75; margin-bottom:10px;">
          Último filtro salvo automaticamente em: <b>${new Date(last._savedAt || Date.now()).toLocaleString("pt-BR")}</b>
        </div>
    ...
    `);
        ...
        $("#btnPesquisar").kendoButton({
            ...
            click: function(){
                ...
                saveLast(payload); // salva SEMPRE o último filtro no clique de Pesquisar

                // Aqui é onde você coloca tua consulta real (API/DataSource)
                // Exemplo: fetch(...), dataSource.filter(...), etc.
            }
        });
        ...
        // Se tiver último filtro, reaplica automaticamente ao abrir
        const last = loadLast();
        if (last) applyFilters(last);
    }

    window.consulta_filtro_init = function(){
        initWidgets();
    };
})();
